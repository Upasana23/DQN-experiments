...
filePath = '' #file path to store model
resultPath = '' #file path to store the results
envName = 'Breakout'
...
def main():
  ...
  logger.log('Iterative Practice DQN with Dense 512. It practice of Dense 512. CNN not locked. epsilon 1.0 to 0.1 5e5. Iter Practice reduced to 1e2. It training 5e5. Viz generated')   
  ...
  for s in range(C['steps']):
    ...
    for it in range(C['iterations']):
      ...
      #Evaluation    
      if (it+(s*500000)+1)%C['eval_freq'] == 0:
